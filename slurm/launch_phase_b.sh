...
#   SKIP_TRAIN         "1" → capture only, do not submit phase_b_train.sbatch
# ─────────────────────────────────────────────────────────────
...
LAUNCH_ENV_FILE="${PROJECT_DIR}/logs/last_phase_b_capture.env"
: > "${LAUNCH_ENV_FILE}"
idx=0
for role in ${ROLES}; do
...
if [[ "${SKIP_TRAIN:-0}" == "1" ]]; then
  echo "SKIP_TRAIN=1 — not submitting phase_b_train.sbatch"
else
