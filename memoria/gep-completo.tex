%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% GEP final
% TFG: Knowledge Distillation y despliegue eficiente de LLMs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
{\LARGE \textbf{Knowledge Distillation y despliegue eficiente de modelos de lenguaje en entornos de supercomputación}}\\[0.5cm]
\\[2.5cm]

{\Large \textbf{Gestión de Proyecto (GEP)}}\\[0.3cm]
{\Large Memoria integrada}\\[3cm]

\begin{center}
\begin{tabular}{r c}
\textbf{Autor:} & Marc Romera Peral \\[0.3cm]
\textbf{Dirección:} & Josep Lluís Berral García / Pol Garcia Recasens \\[0.3cm]
...
El problema central que aborda este TFG es el siguiente: \textbf{¿Cómo reducir el coste computacional de inferencia de los modelos de lenguaje de gran tamaño (LLMs) sin degradar significativamente la calidad de sus respuestas?}
...
\subsection{Relevancia técnica}

Una de las estrategias habituales para reducir el coste de inferencia de los LLMs consiste en aplicar técnicas de compresión de modelos. La \textbf{cuantización} reduce la precisión numérica de los pesos (INT8, INT4) para acelerar cálculos y reducir memoria, aunque puede degradar la calidad de forma difícil de predecir. El \textbf{pruning} elimina conexiones redundantes para obtener modelos sparse más eficientes, pero requiere reentrenamiento y no siempre es compatible con hardware estándar. Técnicas de \textbf{ajuste eficiente} como LoRA permiten adaptar modelos grandes con pocos parámetros entrenables, pero no reducen el coste de inferencia del modelo base.

En este proyecto se adopta \textbf{Knowledge Distillation}~\cite{hinton2015distilling} como técnica habilitadora porque permite disponer de modelos student de distintos tamaños que preservan buena parte del comportamiento del teacher. Esta diversidad de modelos permite construir la solución propuesta en este proyecto, basada en un sistema de inferencia con distintos niveles de coste y capacidad. La novedad de la solución recae en la construcción de un \textbf{sistema de inferencia distribuido} que compara experimentalmente el núcleo mínimo (Políticas A y B) y, si es viable, incorpora la Política C bajo el mismo protocolo de evaluación.

La \textbf{Política~A} (baseline) sirve siempre con el teacher. La \textbf{Política~B} (cascada forzada) intenta primero con el modelo más pequeño y escala progresivamente hasta cumplir un criterio mínimo de calidad. La \textbf{Política~C} (cascada por confianza) utiliza un componente predictivo para estimar qué modelo mínimo cumplirá la calidad deseada y envía la petición directamente a ese modelo. Trabajos recientes como FrugalGPT~\cite{chen2023frugalgpt} y RouteLLM~\cite{dekoninck2024routing} demuestran la viabilidad de estas estrategias.

Como infraestructura se empleará vLLM~\cite{vllm-docs}, un framework de serving con batching continuo y PagedAttention~\cite{vllm2023}. vLLM sirve como plataforma de despliegue y medición en MareNostrum~5.
...
Se adopta la tercera alternativa porque responde a la pregunta de investigación y mantiene como eje el marco teacher--student mediante knowledge distillation. Las otras resuelven parcialmente la eficiencia pero no permiten analizar el comportamiento dinámico de la cascada ni la comparación de políticas. Con acceso a MareNostrum~5, la complejidad adicional es asumible.
...
En la \textbf{Fase~5} se selecciona al menos un student y se obtiene su línea base aplicando el mismo protocolo de evaluación que al teacher. La planificación prioriza la destilación teacher--student como núcleo metodológico; si esta no resulta viable por restricciones de cuota, se activa la contingencia con un modelo preentrenado equivalente y se documenta explícitamente. En la \textbf{Fase~6} se implementan y evalúan las políticas de cascada. La Política~B (cascada forzada) es el mínimo exigible y consiste en intentar primero con el modelo más pequeño y escalar si no cumple el criterio. La Política~C (cascada por confianza) utiliza un componente predictivo basado en señales como log~probs o entropía para decidir a priori qué modelo servir, evitando escalados innecesarios. La Política~C es la versión avanzada, viable si los recursos lo permiten, pero no es requisito indispensable del éxito del proyecto.
...
\addlinespace
Obtención de student por destilación teacher--student (con alternativa preentrenada solo como contingencia) & Optimización a nivel de kernel CUDA \\
\addlinespace
...
\textbf{D4. Informe comparativo de políticas.} Análisis cuantitativo del equilibrio calidad--eficiencia para A y B, incorporando C si su implementación es viable, y contraste teacher--student.

\textbf{D5. Prototipo de cascada.} Implementación y evaluación de la cascada teacher--student (con contingencia preentrenada, si aplica).
...
\textbf{Supuestos.} Acceso a modelos públicos (LLaMA~\cite{touvron2023llama}, Mistral~\cite{jiang2023mistral}) y datasets gratuitos. Compatibilidad vLLM con modelos seleccionados. La destilación teacher--student es la vía principal; si resulta inviable, se activará la contingencia con modelos preentrenados pequeños equivalentes.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 6. METODOLOGÍA DE TRABAJO Y RIGOR
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Metodología de trabajo y rigor}
...
\item \textbf{Sprint Review (final de sprint):} presentar los resultados medibles del sprint (tablas, CSV, logs, scripts) y compararlos con los criterios de aceptación definidos en los objetivos específicos (Sección~3.2).
...
La planificación temporal detallada, con diagrama de Gantt, hitos, dependencias y asignación de recursos, se desarrolla en la sección de planificación temporal de este documento.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TRANSICIÓN A PLANIFICACIÓN TEMPORAL
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Datos globales del proyecto}
...
\subsection{Distribución de las horas de gestión del proyecto (GP)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 4. PLANIFICACIÓN TEMPORAL Y CAMINO CRÍTICO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Planificación temporal y camino crítico}
...
La ejecución de este plan se vehicula mediante \textbf{sprints de dos semanas} dentro del marco Scrum adaptado descrito en la sección de metodología de este documento. En cada Sprint Planning se seleccionan tareas del WBS según prioridad y dependencias, y el progreso se controla mediante issues, milestones y checklists vinculados a los hitos de la Tabla~\ref{tab:hitos}.
...
% Background bar (60h total)
\fill[barlight] (0,0) rectangle (10,0.7);
% Impact bar (44h = 7.33 of 10)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 7. ANÁLISIS CRÍTICO Y COHERENCIA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis crítico y coherencia de la planificación}
...
\midrule
GPU H100 on-demand (AWS \texttt{p5.xlarge})  & 100\,GPU-h\,×\,10,98\,\euro{}/h~\cite{aws2026} & 100\,h & 10,98 & 1.098,00 \\
